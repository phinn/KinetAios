...
- **新增技能 tab** —— 聚合 `~/.kinetaios` / `~/.claude` / `~/.codex` 三方 skills/commands/agents + 插件贡献技能,按来源分组展示、按名/描述搜索(42cd477)
- 点击查看/编辑技能源文件全文(markdown 高亮);用户级技能可直接保存(改名校验、插件技能只读),保存后强制重扫 slash 菜单即时生效

### 🎨 设置交互重构

- **技能编辑改弹层 modal**(c93e577)—— 遮罩 + 居中卡片(CodeEditor 承载),Esc / 点遮罩 / 取消三种关闭;只读态隐藏保存按钮,badge 标注可编辑/只读;四语
- **设置页改左右两栏布局**(2e5c3f9)—— 左侧竖向 tab 导航(选中项左缘 accent 竖条+淡底),右侧内容面板独立滚动,tab 常驻可见(旧 sticky 横向 tab 废除);卡片加宽 760→1080;窄窗口(<900px)自动回退横向换行 tab;命令面板「设置→XX」跳转选择器 `.s-tab[data-stab]` 全兼容;全景搜索跨面板过滤保留
...
- **wecom_approval_list / wecom_approval_detail 工具** —— 智能机器人 WS 通道只有消息收发,审批应用数据在服务端 API 体系;新增 wecom-oa.ts 客户端:corpid+corpsecret → gettoken(7200s 缓存,提前 300s 过期,40014/42001 失效自动重试)→ getapprovalinfo(31 天跨度/100 条/翻页,301112 前置拦截)+ getapprovaldetail
- **审批单列表按 days/时间戳 + 模板/申请人/部门/状态/类型筛选**;详情将 apply_data 控件压平为「标题: 值」行,审批流节点逐人列出(状态/意见/时间),备注列表,sp_status 中文映射
- **设置新增企业微信 OA 区**(corpid/corpsecret,i18n 四语);凭证未配置时工具返回配置指引而非报错崩
- ⚠️ 前置:管理后台「审批-API-审批数据权限」给自建应用授权,否则 301055

### 🧬 替身画像

- **空态支持手动编写** —— 此前只有"生成画像"一条路(编辑器隐藏);现空态新增「手动编写」按钮,就地展开编辑器,与生成共用同一 savePersona 链路(openEditor 统一入口);清空后手写入口重现;i18n 四语
