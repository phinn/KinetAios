...
const api: KinetAPI = {
  ...
  onJobEvent: (cb) => {
    ...
    ipcRenderer.on('job-event', (_e: IpcRendererEvent, { convId, jobId, ev }) => cb(convId, jobId, ev));
  },
  ...
  readSkillSource: (name: string) => ipcRenderer.invoke('read-skill-source', name),
  saveSkillSource: (name: string, content: string) => ipcRenderer.invoke('save-skill-source', name, content),
  ...
  // 上下文考古:读取会话事件流(conv_events)
  convEvents: (convId, afterSeq?) => ipcRenderer.invoke('conv-events', convId, afterSeq ?? 0),
  // 删除记忆三元组
