...
【上下文接力优先级(最重要)】收到"继续/往下做/接着做/好"这类确认型短指令时,它指的是**你上一条回复末尾提出的提议/问句**,判定顺序:
1. 先回看本轮对话尾部:你自己刚提议过什么,就继续做什么。这是默认解释,禁止先去 recall_memory。
2. 用户消息里手贴的关键决策/摘要/报错 = 最高优先级,直接按它干,禁止反问"要哪个/上下文是什么"。
3. 以上都不够 + 需要更早的背景 → recall_memory 搜一轮。
4. **检索为空 ≠ 没有上下文**:工具返回「没有匹配」只说明记忆库里没存过,不代表眼前没有可执行的信息。必须继续执行;实在缺关键细节(路径/表名/要改哪个文件)才提问,且只问缺的那一项,不整体返工。
...
export function sourceHintSection(conv?: Conversation): string {
  ...
  if (conv.wecomKey) {
    return '\n\n# 📱 运行环境:企业微信\n你当前在企业微信中运行。你的回复会自动发送到当前对话,无需任何额外配置。\n如果用户说"发到企微""发给我",那意味着直接回复即可——你已经在企业微信里了。\n\n## 📎 发送文件\n如果用户要求你发送磁盘上已有的文件(如"把 xxx.xlsx 发给我"),请使用 `wecom_send_file` 工具,传入文件路径即可(支持 pdf/xlsx/docx/csv/图片/zip,≤50MB)。不要用 write_file 写二进制文件(会破坏原数据)。注意:纯文本内容直接回复即可,不要为发文本而绕道发文件。';
  }
  return '';
}
      ...
      spawn: async ({ prompt: sub, signal: childSignal, engine, model, scope }) => {
        ...
        try {
          out = await runAgentLoop({
            ...
            ctx: { cwd: conv.cwd, confirm: this.confirm, convId: conv.id, crossProjectMemory: conv.crossProjectMemory === true },
            signal: subAc.signal,
