    ...
    <div class="card">
      ...
      <div class="s-section">
        ...
        <div class="field" style="flex-direction:column;align-items:flex-start;gap:6px">
          <label><input type="checkbox" id="s-mcp-enabled" ${s.localMcpServer?.enabled ? 'checked' : ''} style="width:auto;margin-right:6px" />启用本机 MCP Server(允许其它机器调用我的工具)</label>
          <div style="display:flex;gap:8px;align-items:center">
            <label style="min-width:60px">端口</label>
            <input id="s-mcp-port" type="number" value="${s.localMcpServer?.port ?? 18109}" style="width:80px" />
            <label style="min-width:60px">Token</label>
            <input id="s-mcp-token" type="password" value="${esc(s.localMcpServer?.token ?? '')}" placeholder="留空=不鉴权(仅信任网络)" style="flex:1" />
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <button id="s-mcp-start">立即启动</button>
            <button id="s-mcp-stop">停止</button>
            <span class="test-msg" id="s-mcp-msg"></span>
          </div>
        </div>

        <div class="field" style="flex-direction:column;align-items:flex-start;gap:6px;margin-top:12px">
          <label style="font-weight:600">远程节点(把别的 KinetAios 当工具用)</label>
          <div id="s-remote-list" style="width:100%;display:flex;flex-direction:column;gap:4px"></div>
          <div style="display:flex;gap:8px;align-items:center">
            ...
            <button id="s-remote-add">添加</button>
          </div>
        </div>
      </div>
