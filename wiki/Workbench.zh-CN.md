...
|---|---|---|
| 单位 | 单个会话 | cwd(可能含多个会话) |
| 默认排序 | 最近活动倒序(`updatedAt`) | cwd 字母序 |
| 操作 | 切会话 / 删除 / 重命名 | 切 cwd + 编辑背景 |
| 视图 | 平铺 / 按项目分组(`sb-mode-toggle`) | 永远按 cwd |

侧栏默认按 `updatedAt`(最近活动)倒序排列,每条频道右侧显示相对时间(如"2分钟前")。底部 **🕐** 按钮可切换回创建顺序。详见 [[Session-Management]]。

侧栏的「按项目分组」模式(`▤` 按钮)是 Workbench 的轻量版,只在侧栏里展示分组。
